...
export const SignUpForm = ({ email }: Props) => {
  ...
  return (
    ...
    >
      <h2 className={styles.form__header}>{t("formsHeader")}</h2>
      <Item
        name="name"
      ...
      {/* Need help? */}
      <Link className={styles.form__help_link} to={rootPaths.MAIN.INDEX}>
        {t("needHelp")}
      </Link>
    </Form>
  );
