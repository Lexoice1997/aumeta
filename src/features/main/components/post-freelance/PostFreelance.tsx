...
export const PostFreelanceModal = () => {
	...
	const { data: jobCategories, isLoading: loadingJobCategories } =
		useGetJobCategories(visible)
	const { data: industries, isLoading: loadingIndustries } =
		useGetIndustries(visible)
					...
					component: () => (
						<div className={styles.content}>
							<Flex gap='small'>
								<Item
									name='countryId'
									label={t('homeModals.contactInformation.country')}
								...
								<Item
									name='regionId'
									label={t('homeModals.contactInformation.region')}
									rules={[{ required: true, message: '' }]}
								>
									<Select
							...
							<Flex gap='small'>
								<Item
									name='city'
									label={t('homeModals.contactInformation.city')}
									rules={[{ required: true, message: '' }]}
								>
									<Input />
								</Item>
								<Item
									name='address'
									label={t('homeModals.contactInformation.address')}
									rules={[{ required: true, message: '' }]}
								>
									<Input />
								</Item>
							...
							<Flex gap='small'>
								<PhoneInput
									label={t('homeModals.checkInformation.phoneNumber')}
								/>
								<Item
									name='email'
									label={t('homeModals.contactInformation.email')}
								>
									<Input />
								</Item>
							</Flex>
				...
				},
				{
					...
					component: () => (
						<div className={styles.content}>
							<Item
								name='name'
								label={t('homeModals.internshipInformation.professionTitle')}
							>
								<Input />
							</Item>
							<Flex gap='small'>
								<Item
									name='jobCategoryId'
									label={t('homeModals.internshipInformation.jobCategory')}
								>
									<Select
										suffixIcon={
											<SelectSuffix isLoading={loadingJobCategories} />
										}
										showSearch
										filterSort={(optionA, optionB) =>
											(optionA?.label ?? '')
								...
								<Item
									name='industryId'
									label={t('homeModals.internshipInformation.industry')}
								>
									<Select
							...
							<Flex gap='small'>
								<Item
									name='requirements'
									label={t('homeModals.checkInformation.requirements')}
								>
									<TextArea />
								</Item>
								<Item
									name='responsibilities'
									label={t('homeModals.checkInformation.obligations')}
								>
									<TextArea />
								</Item>
							</Flex>
							<Item
								name='description'
								label={t(
									'homeModals.freelancerInformation.skillsAndExperience'
								)}
							>
								<TextArea />
							</Item>
							<Flex gap='small'>
								<Item
									name='jobTime'
									label={t('homeModals.freelancerInformation.deadline')}
								>
									<DatePicker size='large' placeholder={appT('date')} />
								</Item>
								<Item
									name='salary'
									label={t('homeModals.freelancerInformation.jobSalary')}
								>
									<Input />
								</Item>
							</Flex>
						...
						<div className={styles.content}>
							<div className={styles.content__item}>
								...
								<Flex gap={20}>
									<p>
										{
											countries?.find(el => el.id === state?.countryId)?.name
												?.default
										}{' '}
										{state?.city}
									</p>
							...
							<div className={styles.content__item}>
								...
								<h3>
									{t('homeModals.freelancerInformation.skillsAndExperience')}
								</h3>
								<p>{state?.description}</p>
								<h3>{appT('time')}</h3>
								<Flex gap={8}>
									<p>
										{t('homeModals.freelancerInformation.deadline')}:{' '}
										<span>
											{dayjs(state?.jobTime).format(dayjsFormats.DATE)}
										</span>
									</p>
								</Flex>
							</div>
							<div className={styles.content__item}>
								...
								<Flex gap={8}>
									<p>
										{t('homeModals.checkInformation.phoneNumber')}:{' '}
										<span>{state?.phone}</span>
									</p>
									<p>
										{t('homeModals.contactInformation.email')}:{' '}
							...
							<div className={styles.content__item}>
								...
								<Flex gap={8}>
									<p>
										{t('homeModals.contactInformation.country')}:{' '}
										<span>
											{
												countries?.find(el => el.id === state?.countryId)?.name
													?.default
									...
									<p>
										{t('homeModals.contactInformation.region')}:{' '}
										<span>
											{
												regions?.find(el => el.id === state?.regionId)?.name
													?.default
											}
										</span>
									</p>
									<p>
										{t('homeModals.contactInformation.city')}:{' '}
										<span>{state?.city}</span>
									</p>
								</Flex>
								<Flex>
									<p>
										{t('homeModals.contactInformation.address')}:{' '}
